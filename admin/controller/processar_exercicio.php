...
if (!isset($data['exercicio_id']) || !isset($data['resposta']) || !isset($data['tipo'])) {
...
try {
    ...
    $sql = "SELECT conteudo, tipo FROM exercicios WHERE id = ?";
    ...
    $resultado = processarResposta($resposta_usuario, $conteudo, $tipo);
...
function processarResposta($resposta_usuario, $conteudo, $tipo) {
    switch ($tipo) {
...
function registrarProgresso($conn, $id_usuario, $exercicio_id, $acertou, $pontuacao) {
    ...
    if ($progresso_existente) {
        ...
        $sql_insert = "INSERT INTO progresso_bloco
                       (id_usuario, id_bloco, total_atividades, atividades_concluidas, total_pontos, pontos_obtidos, progresso_percentual, concluido, data_conclusao)
                       VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW())";
        $stmt_insert = $conn->prepare($sql_insert);
        $total_pontos = $total_exercicios * 10;
        $stmt_insert->bind_param("iiiiidii", $id_usuario, $bloco_id, $total_exercicios, $atividades_concluidas, $total_pontos, $pontos_obtidos, $progresso_percentual, $concluido);
        $stmt_insert->execute();
        $stmt_insert->close();
    }
}
