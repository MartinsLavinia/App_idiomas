...
<body>
    ...
    <script>
        ...
        function confirmarNivelFinal(nivel) {
            ...
            // Construir URL de forma mais segura
            const params = new URLSearchParams({
                idioma: '<?php echo htmlspecialchars($idioma_quiz); ?>',
                nivel_escolhido: nivel,
                acertos: <?php echo $acertos; ?>,
                total: <?php echo $total_perguntas; ?>,
                percentual: <?php echo $percentual; ?>
            });
            
            // Redirecionar imediatamente sem delay
            window.location.href = `painel.php?${params.toString()}`;
        }

        // CORREÇÃO: Modificar o evento beforeunload para não bloquear quando o redirecionamento é permitido
        window.addEventListener('beforeunload', function(e) {
            if (loadingOverlay.classList.contains('d-flex') && !redirecionamentoPermitido) {
                e.preventDefault();
                e.returnValue = '';
                return 'Seu progresso está sendo salvo. Tem certeza que deseja sair?';
            }
        });

        // CORREÇÃO: Adicionar fallback caso o redirecionamento falhe
        setTimeout(() => {
            if (loadingOverlay.classList.contains('d-flex') && !redirecionamentoPermitido) {
                loadingOverlay.classList.add('d-none');
                loadingOverlay.classList.remove('d-flex');
                alert('Houve um problema ao redirecionar. Clique novamente para continuar.');
            }
